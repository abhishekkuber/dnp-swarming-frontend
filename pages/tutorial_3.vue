  ...
  export default {
    data() {
      return {
        ...
        isDragging: false,
        tutorialHints: [
          { text: "You can see the top 2 topics for the swarm on the side.", highlightTarget: null },
          { text: "This process will be repeated again for the last time.", highlightTarget: null },
          { text: "Again, the group's decision is clear in this case", highlightTarget: 'poi-1'},
          { text: "However, when the swarm chooses a topic in this round, the process is over!", highlightTarget: null },   
        ...
        if (!this.intervalId) {
          this.intervalId = setInterval(() => {
            if (this.socket && this.mouseEnabled) {
              this.socket.emit('tutorial-3-mouse-move', { room: this.currentRoom, cursorPosition: this.cursorPosition, puckPosition: this.puckPosition});
            }
          }, 100);
        }
      },
